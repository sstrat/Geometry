...
struct Button {
    ...
    void putButton(Shader shader) {
        ...
        RenderText(shader, name, (x1 + x2) / 2 - textWidth(name, scale) / 2 - textWidth(name.substr(0,1), scale)/2, (y1 + y2) / 2 - textHeight(name, scale) / 2, scale, glm::vec3(0, 0, 0));
        glUseProgramObjectARB(0);
    }
};
...
vector<Button> makeButtons(int width, int height) {
    ...
    buttons[0].name = "point";
    buttons[1].name = "shape";
    buttons[2].name = "delete";
    buttons[3].name = "path";
    buttons[4].name = "hull";
    buttons[5].name = "domain";
    buttons[5].scale = 0.6;
